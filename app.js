...
window.addEventListener('beforeinstallprompt', (e) => {
    // منع المتصفح من إظهار نافذة التثبيت التلقائية الافتراضية
    e.preventDefault();
    // حفظ الحدث للاستجابة له عند النقر
    deferredPrompt = e;
    
    // إظهار زر التنزيل إن كان مخفياً
    let installBtn = document.getElementById('install-btn');
    if (installBtn) {
        installBtn.style.display = 'inline-block';
    }
    console.log('تم التقاط حدث التثبيت بنجاح (beforeinstallprompt)');
});
...
async function installApp() {
    ...
    if (deferredPrompt) {
        ...
        // انتظار رد المستخدم (موافقة أو رفض)
        const { outcome } = await deferredPrompt.userChoice;
        console.log(`قرار المستخدم من التثبيت: ${outcome}`);
        
        if (outcome === 'accepted') {
            // إخفاء الزر وإظهار ملاحظة النسخة بعد نجاح التثبيت الفعلي
            ...
            if (!existingNotice && startBtn) {
                ...
                noticeElem.innerText = "نسخة تجريبية مثبتة";
                startBtn.insertAdjacentElement('afterend', noticeElem);
            }
        }
        // لا يمكن استخدام الحدث مرتين، يتم تصفيره
        deferredPrompt = null;
    } else {
        // تنبيه ذكي إذا كان المتصفح لا يدعم التثبيت المباشر أو التطبيق مثبت مسبقاً
        alert("التثبيت غير متاح حالياً. تأكد من أنك تستخدم متصفحاً داعماً (مثل Chrome أو Edge أو Safari) أو أن التطبيق مثبت مسبقاً على جهازك.");
        
        // إظهار الملاحظة التجريبية لتأكيد الاستجابة
        if (installBtn) installBtn.style.display = 'none';
        let startBtn = document.getElementById('start-btn') || installBtn.parentElement;
        let existingNotice = document.getElementById('trial-version-notice');
        if (!existingNotice && startBtn) {
