...
function updateReviewView() {
    ...
    // 1. الآية الرئيسية (الخضراء) - مخفية صراحة في البداية
    let blankBox1 = document.getElementById("target-blank-1");
    if(blankBox1) {
        blankBox1.innerText = " " + nextAyah1.text + " ";
        blankBox1.style.visibility = 'hidden'; // إخفاء صريح لئلا تظهر قبل الضغط
        blankBox1.style.color = "#16a34a"; // اللون الأخضر عند الكشف
    ...
    // 2. الآية الموالية (أول 3 كلمات) - مخفية صراحة باللون الرمادي ولن تظهر إلا عند الضغط
    ...
    if(blankBox2 && nextAyah2) {
        let words = nextAyah2.text.trim().split(/\s+/);
        let firstThreeWords = words.slice(0, 3).join(" ");
        
        let isCompleteAyah = words.length <= 3;
        let displaySuffix = isCompleteAyah ? "" : " ...";
        
        blankBox2.innerText = " " + firstThreeWords + displaySuffix + " ";
        blankBox2.style.visibility = 'hidden'; // إخفاء صريح لتظل مخفية في البداية
        blankBox2.style.color = "#4b5563"; // تثبيت اللون الرمادي حصراً
